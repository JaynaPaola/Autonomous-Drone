...
image_paths = [
    "1de11fa6-f58a-4c19-988f-dd5ddc3467f9.jpg",   # img 1
    "ced57e85-47d1-482e-88eb-a9860d15da24.jpg",   # img 2
    "0279053f-c683-4fb7-85f8-32ca828f9dd6.jpg",   # img 3
    "resultado_final.jpg",   # img 5
    "5f12e1d5-c055-4950-b388-cd282c2c474d.jpg",   # img 4
    "e20f924e-9bf5-4bda-bd04-f2a58f0a9123.jpg",   # img 6
]
...
# ─────────────────────────────────────────────────────────────────────────────
# Funciones base (sin cambios respecto al código original)
# ─────────────────────────────────────────────────────────────────────────────

def detect_features(image_bgr, n_features=3000):
    """Detecta keypoints y calcula descriptores ORB."""
...
def match_features(desc1, desc2, ratio=0.75):
    """BFMatcher Hamming + prueba de ratio de Lowe."""
...
def compute_homography(kp_src, kp_dst, matches, reproj_thresh=4.0):
    """Homografía H (dst ≈ H·src) con RANSAC."""
    if len(matches) < 4:
        raise ValueError(f"Se necesitan ≥4 matches, se tienen {len(matches)}")
...
def warp_and_blend(img_src, img_dst, H):
    """Proyecta img_src sobre img_dst y devuelve la imagen fusionada."""
    ...
    # Guardar región de img_dst sin pisar píxeles ya warpeados
    roi = warped[-y_min:-y_min + h_dst, -x_min:-x_min + w_dst]
    mask_dst = np.all(roi == 0, axis=2)          # píxeles negros (vacíos)
    roi[~mask_dst] = img_dst[~mask_dst]           # conserva warped donde hay contenido
    roi[mask_dst]  = img_dst[mask_dst]            # rellena vacíos con img_dst

    return warped


# ─────────────────────────────────────────────────────────────────────────────
# Función que une UN par de imágenes y guarda el resultado
# ─────────────────────────────────────────────────────────────────────────────

def stitch_pair(img_a, img_b, label, show=True):
    """
    Une img_a (izquierda) con img_b (derecha).
    Guarda el resultado en disco y opcionalmente lo muestra.
    ...
    # Guardar en disco
    filename = label.replace(" ", "_").replace("+", "-") + ".jpg"
...
print("\n\n=== GRUPO B: imagenes 4 y 6 ===")

P46 = stitch_pair(img4, img6, "P46")
...
print(f"    Grupo A -> {os.path.join(OUTPUT_DIR, 'P12.jpg')}")
print(f"    Grupo A -> {os.path.join(OUTPUT_DIR, 'P123.jpg')}")
print(f"    Grupo B -> {os.path.join(OUTPUT_DIR, 'P46.jpg')}")
print(f"{'*'*50}\n")